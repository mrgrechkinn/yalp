...
def execute(**kargs):
    ...
    add_options = ['-Dapplication.path=%s' % (app.path), '-Dframework.path=%s' % (yalp_env['basedir']), '-Dyalp.id=%s' % yalp_env['id'], '-Dyalp.version=%s' % yalp_env['version']]
    if args.count('--jpda'):
    ...
    java_cmd = [app.java_path()] + add_options + ['-classpath', app.cp_args(), 'yalp.db.Evolutions']
