...
%{
    ...
    if(! _name) {
        throw new yalp.exceptions.TagInternalException("name attribute cannot be empty for select tag");
    }

    if(!_valueProperty)
        _valueProperty = 'id';
    yalp.templates.TagContext.current().data.put("selected", _value);
    
    serializedAttrs  = yalp.templates.FastTags.serialize(_attrs, "size", "name", "items", "labelProperty", "value", "valueProperty")
}%

<select name="${_name}" size="${_size?:1}" ${serializedAttrs}>
    #{doBody /}
    #{list items:_items, as:'i'}
        #{option _valueProperty && i.hasProperty(_valueProperty) ? i[_valueProperty] : i}&{_labelProperty && i.hasProperty(_labelProperty) ? yalp.utils.HTML.htmlEscape(i[_labelProperty]) : i}#{/option}
    #{/list}
</select>
