...
%{
    ...
    if (!_src) {
        throw new yalp.exceptions.TagInternalException("src attribute cannot be empty for stylesheet tag");
    }
    _src = "/public/stylesheets/" + _src
    try {
        _abs = yalp.mvc.Router.reverseWithCheck(_src, yalp.Yalp.getVirtualFile(_src), false);
    } catch (Exception ex) {
        throw new yalp.exceptions.TagInternalException("File not found: " + _src);
    }}%
